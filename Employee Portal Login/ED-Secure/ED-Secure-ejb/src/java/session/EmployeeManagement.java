...
public class EmployeeManagement implements EmployeeManagementRemote {
    ...
    public EmployeeDTO getUserDetails() {
        ...
        if (callerKey == null) {
            // no such employee
            return null;
        } else if (callerKey.equals("ed-user1")) {
            Employee e = employeeFacade.find("00003");

            EmployeeDTO empDTO = new EmployeeDTO(e.getEmpid(),
                    e.getName(), e.getPhone(), e.getAddress(),
                    e.getEmail(), e.getPassword(), e.getAppGroup(),
                    e.getBnkAccId(), e.getSalary(), e.isActive());

            return empDTO;

        } else if (callerKey.equals("ed-user2")) {
            Employee e = employeeFacade.find("00004");
